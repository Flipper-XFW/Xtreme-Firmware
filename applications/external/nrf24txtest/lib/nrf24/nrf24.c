...
bool nrf24_checkconnected(FuriHalSpiBusHandle* handle) {
    ...
    // FURI_LOG_I(appname, "status: %02x", status);
...
void nrf24_start_testtx(uint8_t ch, FuriHalSpiBusHandle* handle) {
    // set registers  https://infocenter.nordicsemi.com/pdf/nan_24-08.pdf  2.1.1
    nrf24_write_reg(handle, R_REGISTER, 0x02);
    nrf24_write_reg(handle, REG_RF_CH, ch);
    nrf24_write_reg(handle, REG_RF_SETUP, 0x9F);
}

void nrf24_stop_testtx(FuriHalSpiBusHandle* handle) {
    // reset registers
    nrf24_write_reg(handle, R_REGISTER, 0x08);
    nrf24_write_reg(handle, REG_RF_CH, 0x02);
    nrf24_write_reg(handle, REG_RF_SETUP, 0x0F);
}